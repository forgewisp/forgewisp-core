...
// Loads the subagent-demo's index.html body into the jsdom document so the static
// a11y attributes are present. main.ts is NOT imported — this test only asserts
// the markup shipped in index.html.
...
describe('a11y — index.html attributes', () => {
  ...
  it('artifacts list has an aria-label', () => {
    ...
    expect(list?.getAttribute('aria-label')).toBe('Subagent runs');
  });

...
function getEl<T extends HTMLElement>(id: string): T {
  ...
  if (!el) throw new Error(`[bundled-demo] Missing element #${id}`);
  return el as T;
}
...
function maybeRenderArtifact(event: AuditEvent): void {
  const html = renderArtifact(event);
  if (!html) return;
  const li = document.createElement('li');
  li.className =
    event.type === 'function_errored'
      ? 'artifact artifact-error'
      : `artifact artifact-${event.functionName}`;
  li.innerHTML = html;
...
function buildAgent(cfg: AgentConfig): void {
  ...
  const config: ForgewispConfig = {
    ...
    model: cfg.model,
    systemPrompt:
      'You are a browser-effects assistant with a toolkit of read and side-effect tools: ' +
      'current time, UUIDs, safe math, hashing, base64, viewport/battery info, geolocation, ' +
      'clipboard, speech synthesis, file download, and localStorage. Always explain briefly ' +
      'what you are about to do before calling a write/destructive tool, and prefer the ' +
      'least-invasive tool. Chain tools when the request needs several steps.',
...
// `BUNDLED_TOOLS` is a heterogeneous readonly tuple of `FunctionDefinition<SpecificArgs>`
// for various arg shapes. Iterating it yields the union of those types, which the
// generic `registerFunction<TArgs>` can't infer a single `TArgs` for (the handler is
// contravariant, and interfaces like `EvaluateMathArgs` aren't assignable to
// `Record<string, unknown>`). Erase to the defaulted `FunctionDefinition` once —
// the core itself casts to this same type when storing in the registry.
const TOOLS = BUNDLED_TOOLS as unknown as readonly FunctionDefinition[];

// Register every bundled tool. BUNDLED_TOOLS includes write/destructive tools,
// so `onConfirmRequired` (configured above) is required for registration to
// succeed — the core throws at registration time otherwise.
...
els.configForm.addEventListener('submit', (e: SubmitEvent) => {
  ...
  localStorage.setItem('forgewisp.bundled-demo.config', JSON.stringify(cfg));
  buildAgent(cfg);
  hideConfigForm();
});
...
function loadStoredConfig(): AgentConfig | null {
  const stored = localStorage.getItem('forgewisp.bundled-demo.config');
  ...
  if (!isAgentConfig(parsed)) {
    ...
    localStorage.removeItem('forgewisp.bundled-demo.config');
    return null;
  }
  return parsed;
}
...
els.toolsList.innerHTML = renderToolsList(BUNDLED_TOOLS);
